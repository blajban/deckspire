import {
  HexGridOrientation,
  HorizontalOrientationEdge,
  VectorHex,
  VerticalOrientationEdge,
} from '../../src/math/VectorHex';
...
describe('Creating VectorHex', () => {
  ...
  test('From Vector2D', () => {
    ...
    vec = VectorHex.from_vector2D({
      x: (1.001 * Math.cos(Math.PI / 6)) / Math.sqrt(2),
      y: (1.001 * Math.sin(Math.PI / 6)) / Math.sqrt(2),
    });
    expect(vec.q).toBe(1);
    expect(vec.r).toBe(0);
    vec = VectorHex.from_vector2D({
      x: (0.999 * Math.cos(Math.PI / 6)) / Math.sqrt(2),
      y: (0.999 * Math.sin(Math.PI / 6)) / Math.sqrt(2),
    });
    expect(vec.q).toBe(0);
    expect(vec.r).toBe(0);
  });
...
describe('Converting VectorHex', () => {
  ...
  test('Hex to Vector2D with scaling', () => {
    expect(() =>
      new VectorHex(1, 0).into_vector2d(HexGridOrientation.Horizontal, 0),
