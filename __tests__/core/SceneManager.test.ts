...
describe('SceneManager', () => {
  ...
  test('should load a registered scene', () => {
    ...
    scene_manager.loadScene(ecs, 'Scene1');
    expect(mock_scene.events).toContain('load');
  });

  test('should not load an unregistered scene', () => {
    expect(() => scene_manager.loadScene(ecs, 'UnknownScene')).toThrow(
      'Cannot load scene UnknownScene, it is not registered.',
    );
  });
  ...
  test('should not load an already loaded scene', () => {
    ...
    expect(() => scene_manager.loadScene(ecs, 'Scene1')).toThrow(
