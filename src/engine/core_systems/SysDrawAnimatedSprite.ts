...
export default class SysDrawAnimatedSprite extends System {
  ...
  update(
    ...
    ecs
      ...
      .forEach(([drawable, sprite, transform], _entity) => {
        const image_asset = ecs.asset_store!.getAsset(
          sprite!.states.current_state.asset_id!,
        );
        ...
        if (!phaser_context.phaser_scene.anims.exists(current_state!.key)) {
          phaser_context.phaser_scene!.anims.create({
            ...
            frames: phaser_sprite.anims.generateFrameNumbers(
              ecs.asset_store!.getAsset(current_state!.asset_id!),
              {
            ...
            repeat: current_state!.should_loop ? -1 : 0,
          });
        }

        if (
          !phaser_sprite.anims.isPlaying ||
          phaser_sprite.anims.currentAnim?.key !== current_state?.key
        ) {
          phaser_sprite.play(current_state!.key);
        }

        if (!current_state.should_loop) {
          phaser_sprite.once(
            'animationcomplete',
